...
def diagonally_combine_two_images(img1, img2, x_threshold, angle, gap=0, color=1):
    ...
    if gap > 0:
        rr, cc, val = weighted_line(0, int(x_threshold * img1.shape[1] - (angle * img1.shape[0] / 2)), img1.shape[0] - 1, int(
            x_threshold * img1.shape[1] + (angle * img1.shape[0] / 2)), gap)
...
def read_image(file):
    if os.path.splitext(file)[1] == ".bin":
        with open(file, "rb") as f:
            ...
            img = np.frombuffer(bytes, dtype=np.float16, count=h * w * 4,
