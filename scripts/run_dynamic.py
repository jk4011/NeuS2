...
def parse_args():
    ...
    parser.add_argument("--mode", default="", const="nerf", nargs="?", choices=[
                        "nerf", "sdf", "image", "volume"], help="Mode can be 'nerf', 'sdf', or 'image' or 'volume'. Inferred from the scene if unspecified.")
    ...
    parser.add_argument("--save_snapshot_per_frame", action="store_true",
                        help="Save this snapshot after training every frame. recommended extension: .msgpack")

    parser.add_argument("--train_from_frame", default="",
                        help="Save this snapshot after training every frame. recommended extension: .msgpack")
    parser.add_argument("--test_from_frame", default="",
                        help="Save this snapshot after training every frame. recommended extension: .msgpack")
    ...
    parser.add_argument("--save_mesh_path", default="",
    ...
    if args.dynamic_test:
        ...
        while True:
            ...
            if args.load_snapshot != "":
                load_snapshot_path = os.path.join(args.load_snapshot, "checkpoints") + \
                    f"/frame_{testbed.current_training_time_frame}.msgpack"
            else:
                load_snapshot_path = os.path.join(args.output_path, "checkpoints") + \
                    f"/frame_{testbed.current_training_time_frame}.msgpack"
            ...
            if args.dynamic_save_mesh:
                ...
                save_mesh_path = os.path.join(args.output_path, "evaluation", "mesh",
            ...
            log_ptr, avg_psnr_this_frame = cal_psnr(testbed, ref_images, 1, True, save_dir=os.path.join(
                args.output_path, "evaluation", "psnr", f"frame_{testbed.current_training_time_frame}"), spp=8, white_bkgd=args.white_bkgd)
        ...
        if n_steps > 0:
            ...
            while testbed.frame():
                ...
                if (testbed.current_training_time_frame == 0 and testbed.training_step == testbed.first_frame_max_training_step) or (testbed.current_training_time_frame != 0 and testbed.training_step == testbed.next_frame_max_training_step):
                    ...
                    if args.save_mesh:
                        res = args.marching_cubes_res
                        save_mesh_path = os.path.join(args.output_path, "mesh",
                                                      f"frame_{testbed.current_training_time_frame:04}.obj")
                        print(
                            f"Generating mesh via marching cubes and saving to {save_mesh_path}. Resolution=[{res},{res},{res}]")
                    ...
                    pred_img, gt_img, pred_mesh = render_img_training_view(
                        args, testbed, eval_log_ptr, all_transform_path[testbed.current_training_time_frame], testbed.current_training_time_frame)
                    ...
                    if testbed.current_training_time_frame != testbed.all_training_time_frame - 1:
                        writer_frame = SummaryWriter(log_dir=os.path.join(
                            'output', args.name, 'logs', time_name, f'frame_{testbed.current_training_time_frame + 1}'))
                        if args.test_psnr:
